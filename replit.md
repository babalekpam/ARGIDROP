...
**4. Mobile Driver/Merchant App (Expo):**
    ...
    *   **iOS submission flow (27 April 2026):** App Store Connect listing "ARGIDROP" is live (SKU `argidrop-driver-001`, French primary, bundle id `com.argilette.argidrop.driver`, Apple Team ID `Z82KB5AR7N`, Issuer ID `d6b6d45d-83aa-47cb-87e2-827015b47527`). Auth uses an App Store Connect API Key (.p8). Three secrets are required: `EXPO_ASC_API_KEY_ID`, `EXPO_ASC_API_KEY_ISSUER_ID`, `EXPO_ASC_API_KEY_P8` (full file contents including BEGIN/END lines). Submission is wired through two scripts that materialize the .p8 to a tmp dir with mode 600 then clean up on exit: `mobile/scripts/build-ios.sh` (kicks off EAS production iOS build, non-interactive, --no-wait) and `mobile/scripts/submit-ios.sh` (submits the latest production build). **`eas submit` does NOT accept `--api-key-*` CLI flags** — credentials are passed through the canonical EAS env vars exported by the script: `EXPO_ASC_API_KEY_PATH`, `EXPO_ASC_KEY_ID`, `EXPO_ASC_ISSUER_ID`. `appleTeamId` and `ascAppId` live in `mobile/eas.json` under `submit.production.ios` (ascAppId is currently a placeholder `REPLACE_WITH_NUMERIC_APPLE_ID_FROM_ASC` — fetch the numeric Apple ID from ASC → My Apps → ARGIDROP → App Information and replace before submitting). `.p8` files are gitignored. **Security note:** the original .p8 (Key ID `MMRCUUPD72`) was leaked via chat attachment on 27 April 2026 and was REVOKED in App Store Connect; do not reuse Key ID `MMRCUUPD72`. Never accept .p8 file contents via chat — always via Replit Secrets.
    *   **Apple payment-review hardening (27 April 2026):** All in-app payment copy (`mobile/src/screens/merchant/PaymentSheetScreen.js`) is fully French via the existing `useLang` / `t()` i18n layer (~22 `payment.*` keys in `mobile/src/utils/i18n.js`). The amber `SANDBOX` debug badge is gated behind `__DEV__` so production builds (TestFlight/App Store) never show it. The backend demo-checkout fallback (`backend/src/routes/payments.js` GET/POST `/demo/confirm`) is **disabled in production** (`NODE_ENV === 'production'` returns 410 Gone for both verbs), and `backend/src/services/payment-providers/index.js` filters out any non-live adapter from `/payments/providers` in production. Together this guarantees an App Store reviewer (or any production user) cannot land on a simulated-payment page — they only ever see real provider checkouts when the country has live keys, otherwise the merchant sees the standard "no providers available" message. In dev/staging the demo path remains active for internal QA, with a French `MODE DÉVELOPPEMENT — paiement simulé` badge.
