// Bilingual strings — French (fr) and English (en)
// Usage: t('selfie.title', lang)

const strings = {
  // ─── COMMON ───
  'common.continue': { fr: 'Continuer', en: 'Continue' },
  'common.back': { fr: 'Retour', en: 'Back' },
  'common.save': { fr: 'Enregistrer', en: 'Save' },
  'common.cancel': { fr: 'Annuler', en: 'Cancel' },
  'common.submit': { fr: 'Soumettre', en: 'Submit' },
  'common.retry': { fr: 'Réessayer', en: 'Try again' },
  'common.uploading': { fr: 'Envoi en cours…', en: 'Uploading…' },
  'common.saving': { fr: 'Sauvegarde…', en: 'Saving…' },
  'common.loading': { fr: 'Chargement…', en: 'Loading…' },
  'common.required': { fr: 'Requis', en: 'Required' },
  'common.optional': { fr: 'Optionnel', en: 'Optional' },
  'common.uploaded': { fr: '✓ Envoyé', en: '✓ Uploaded' },
  'common.approved': { fr: '✓ Approuvé', en: '✓ Approved' },
  'common.rejected': { fr: 'À corriger', en: 'Re-upload needed' },
  'common.pending': { fr: 'En attente', en: 'Under review' },
  'common.replace': { fr: 'Remplacer', en: 'Replace file' },
  'common.choose': { fr: 'Choisir le fichier', en: 'Choose file' },
  'common.remove': { fr: 'Supprimer', en: 'Remove' },
  'common.apply': { fr: 'Appliquer', en: 'Apply' },

  // ─── ONBOARDING STEPS ───
  'onboarding.step': { fr: 'Étape', en: 'Step' },
  'onboarding.of': { fr: 'sur', en: 'of' },
  'onboarding.vehicle.title': { fr: 'Votre véhicule', en: 'Your vehicle' },
  'onboarding.vehicle.subtitle': { fr: 'Décrivez votre moyen de livraison', en: 'Tell us what you drive' },
  'onboarding.payout.title': { fr: 'Compte de paiement', en: 'Payout account' },
  'onboarding.payout.subtitle': { fr: 'Où envoyer vos gains après chaque livraison ?', en: 'Where should we send your earnings?' },
  'onboarding.kyc.title': { fr: 'Vérification d\'identité', en: 'Identity verification' },
  'onboarding.kyc.subtitle': { fr: 'Nous vérifions chaque livreur pour protéger nos clients', en: 'We verify every driver to protect our customers' },

  // ─── SELFIE ───
  'selfie.step': { fr: 'Selfie en direct', en: 'Live selfie' },
  'selfie.title': { fr: 'Photo de votre visage', en: 'Your face photo' },
  'selfie.subtitle': { fr: 'Prenez une photo nette de votre visage. Cette photo sera visible par les entreprises clientes.', en: 'Take a clear photo of your face. Businesses will see this on your profile.' },
  'selfie.instructions.title': { fr: 'Conseils pour une bonne photo', en: 'Tips for a good photo' },
  'selfie.instruction.1': { fr: 'Bonne lumière, visage bien visible', en: 'Good lighting, face clearly visible' },
  'selfie.instruction.2': { fr: 'Pas de lunettes de soleil, pas de chapeau', en: 'No sunglasses, no hat' },
  'selfie.instruction.3': { fr: 'Fond neutre (mur blanc ou clair)', en: 'Neutral background (white or light wall)' },
  'selfie.instruction.4': { fr: 'Regardez droit vers l\'appareil photo', en: 'Look directly at the camera' },
  'selfie.take': { fr: 'Prendre la photo', en: 'Take photo' },
  'selfie.retake': { fr: 'Reprendre', en: 'Retake' },
  'selfie.confirm': { fr: 'Confirmer la photo', en: 'Confirm photo' },
  'selfie.with_id.step': { fr: 'Selfie avec pièce d\'identité', en: 'Selfie with ID' },
  'selfie.with_id.title': { fr: 'Tenez votre pièce d\'identité', en: 'Hold your ID document' },
  'selfie.with_id.subtitle': { fr: 'Prenez une photo de vous tenant votre CNI ou passeport ouvert, bien lisible.', en: 'Take a photo of yourself holding your open, readable national ID or passport.' },
  'selfie.with_id.instruction.1': { fr: 'Tenez la pièce d\'identité à hauteur de votre poitrine', en: 'Hold ID at chest height' },
  'selfie.with_id.instruction.2': { fr: 'Les deux visages (vôtre et sur la pièce) doivent être visibles', en: 'Both faces (yours and on ID) must be visible' },
  'selfie.with_id.instruction.3': { fr: 'Le texte de la pièce doit être lisible', en: 'Text on ID must be readable' },

  // ─── KYC DOCUMENTS ───
  'kyc.title': { fr: 'Documents requis', en: 'Required documents' },
  'kyc.subtitle': { fr: 'Tous les documents doivent être clairs et en cours de validité', en: 'All documents must be clear and currently valid' },
  'kyc.submit': { fr: 'Soumettre pour vérification', en: 'Submit for review' },
  'kyc.submit.disabled': { fr: 'Envoyez tous les documents pour soumettre', en: 'Upload all documents to submit' },
  'kyc.pending.title': { fr: 'Dossier en cours d\'examen', en: 'Application under review' },
  'kyc.pending.subtitle': { fr: 'Notre équipe examine généralement les dossiers sous 24 heures. Vous recevrez une notification dès que votre compte sera validé.', en: 'Our team typically reviews applications within 24 hours. You\'ll be notified when your account is verified.' },
  'kyc.rejected.title': { fr: 'Documents à corriger', en: 'Documents need correction' },
  'kyc.rejected.subtitle': { fr: 'Un ou plusieurs documents ont été refusés. Veuillez les corriger.', en: 'One or more documents were rejected. Please correct them.' },

  // ─── DOCUMENT LABELS ───
  'doc.SELFIE.label': { fr: 'Photo de profil (selfie)', en: 'Profile photo (selfie)' },
  'doc.SELFIE.hint': { fr: 'Photo nette de votre visage — visible par les clients', en: 'Clear face photo — visible to customers' },
  'doc.SELFIE_WITH_ID.label': { fr: 'Selfie avec pièce d\'identité', en: 'Selfie holding ID' },
  'doc.SELFIE_WITH_ID.hint': { fr: 'Votre visage + CNI ou passeport tenu dans la main', en: 'Your face + national ID or passport held in hand' },
  'doc.GOVT_ID_FRONT.label': { fr: 'CNI / Passeport — Recto', en: 'National ID / Passport — Front' },
  'doc.GOVT_ID_FRONT.hint': { fr: 'Carte nationale d\'identité ou passeport, côté recto', en: 'National ID card or passport, front side' },
  'doc.GOVT_ID_BACK.label': { fr: 'CNI — Verso', en: 'National ID — Back' },
  'doc.GOVT_ID_BACK.hint': { fr: 'Côté verso de votre carte nationale d\'identité', en: 'Back side of your national ID card' },
  'doc.DRIVERS_LICENSE.label': { fr: 'Permis de conduire', en: 'Driver\'s license' },
  'doc.DRIVERS_LICENSE.hint': { fr: 'Permis valide pour la catégorie de véhicule utilisée', en: 'Valid license for the vehicle category you use' },
  'doc.VEHICLE_REGISTRATION.label': { fr: 'Carte grise', en: 'Vehicle registration' },
  'doc.VEHICLE_REGISTRATION.hint': { fr: 'Certificat d\'immatriculation du véhicule à votre nom', en: 'Vehicle registration certificate in your name' },
  'doc.VEHICLE_INSURANCE.label': { fr: 'Attestation d\'assurance', en: 'Insurance certificate' },
  'doc.VEHICLE_INSURANCE.hint': { fr: 'Assurance en cours de validité (non expirée)', en: 'Currently valid insurance (not expired)' },
  'doc.VEHICLE_PHOTO_FRONT.label': { fr: 'Photo du véhicule (plaque visible)', en: 'Vehicle photo (plate visible)' },
  'doc.VEHICLE_PHOTO_FRONT.hint': { fr: 'Photo de face montrant clairement la plaque d\'immatriculation', en: 'Front-facing photo clearly showing the license plate' },
  'doc.POLICE_CLEARANCE.label': { fr: 'Casier judiciaire (bulletin n°3)', en: 'Police clearance certificate' },
  'doc.POLICE_CLEARANCE.hint': { fr: 'Extrait de casier judiciaire datant de moins de 3 mois', en: 'Criminal record certificate less than 3 months old' },
  'doc.PROOF_OF_ADDRESS.label': { fr: 'Justificatif de domicile', en: 'Proof of address' },
  'doc.PROOF_OF_ADDRESS.hint': { fr: 'Facture d\'eau, d\'électricité ou téléphone fixe, datant de moins de 3 mois', en: 'Water, electricity, or landline phone bill less than 3 months old' },

  // ─── VEHICLE TYPES ───
  'vehicle.BICYCLE': { fr: 'Vélo', en: 'Bicycle' },
  'vehicle.MOTORCYCLE': { fr: 'Moto', en: 'Motorcycle' },
  'vehicle.TRICYCLE': { fr: 'Tricycle', en: 'Tricycle' },
  'vehicle.CAR': { fr: 'Voiture', en: 'Car' },
  'vehicle.VAN': { fr: 'Camionnette', en: 'Van' },
  'vehicle.TRUCK': { fr: 'Camion', en: 'Truck' },

  // ─── VEHICLE FIELDS ───
  'vehicle.type': { fr: 'Type de véhicule', en: 'Vehicle type' },
  'vehicle.make': { fr: 'Marque', en: 'Make' },
  'vehicle.model': { fr: 'Modèle', en: 'Model' },
  'vehicle.year': { fr: 'Année', en: 'Year' },
  'vehicle.color': { fr: 'Couleur', en: 'Color' },
  'vehicle.plate': { fr: 'Plaque d\'immatriculation', en: 'License plate' },

  // ─── PAYOUT ───
  'payout.provider': { fr: 'Opérateur mobile money', en: 'Mobile money provider' },
  'payout.account': { fr: 'Numéro de paiement', en: 'Payout phone number' },
  'payout.hint': { fr: 'Vos gains seront envoyés à ce numéro après chaque livraison', en: 'Your earnings will be sent to this number after each delivery' },

  // ─── ERRORS ───
  'error.camera': { fr: 'Accès à la caméra refusé. Activez-le dans les paramètres.', en: 'Camera access denied. Enable it in settings.' },
  'error.upload': { fr: 'Échec de l\'envoi. Veuillez réessayer.', en: 'Upload failed. Please try again.' },
  'error.missing_docs': { fr: 'Tous les documents sont obligatoires', en: 'All documents are required' },
  'error.vehicle_type': { fr: 'Sélectionnez votre type de véhicule', en: 'Select your vehicle type' },
  'error.payout': { fr: 'L\'opérateur et le numéro de paiement sont requis', en: 'Payout provider and account are required' },

  // ─── ROLE SELECT ───
  'role.welcome': { fr: 'Bienvenue', en: 'Welcome' },
  'role.howUse': { fr: 'Comment utiliserez-vous ArgiDrop aujourd\'hui ?', en: 'How will you use ArgiDrop today?' },
  'role.merchant.title': { fr: 'Je suis Marchand', en: 'I\'m a Merchant' },
  'role.merchant.desc': { fr: 'Envoyez des colis, gérez les livraisons de votre boutique, suivez les commandes en direct.', en: 'Send packages, manage your shop\'s deliveries, track orders live.' },
  'role.driver.title': { fr: 'Je suis Livreur', en: 'I\'m a Driver' },
  'role.driver.desc': { fr: 'Gagnez de l\'argent à votre rythme. Soyez payé chaque jour sur mobile money.', en: 'Earn money on your schedule. Get paid daily to mobile money.' },
  'role.alreadyAccount': { fr: 'Vous avez déjà un compte ?', en: 'Already have an account?' },
  'role.signin': { fr: 'Se connecter', en: 'Sign in' },
  'role.brandSub': { fr: 'Lomé · Afrique de l\'Ouest', en: 'Lomé · West Africa' },

  // ─── LOGIN ───
  'login.title': { fr: 'Se connecter', en: 'Sign in' },
  'login.subtitle': { fr: 'Bon retour', en: 'Welcome back' },
  'login.email': { fr: 'Adresse e-mail', en: 'Email address' },
  'login.password': { fr: 'Mot de passe', en: 'Password' },
  'login.submit': { fr: 'Se connecter', en: 'Sign in' },
  'login.newDriver': { fr: 'Nouveau ?', en: 'New here?' },
  'login.registerHere': { fr: 'Inscrivez-vous ici', en: 'Register here' },
  'login.requiredFields': { fr: 'Champs requis', en: 'Required fields' },
  'login.requiredFieldsMsg': { fr: 'Veuillez saisir votre e-mail et votre mot de passe', en: 'Please enter your email and password' },
  'login.failed': { fr: 'Échec de la connexion', en: 'Login failed' },
  'login.invalidCreds': { fr: 'Identifiants invalides', en: 'Invalid credentials' },

  // ─── REGISTER ───
  'register.merchantSub': { fr: 'Inscription marchand', en: 'Merchant registration' },
  'register.driverSub': { fr: 'Inscription livreur', en: 'Driver registration' },
  'register.title': { fr: 'Créer un compte', en: 'Create account' },
  'register.merchantSubtitle': { fr: 'Rejoignez les milliers de marchands qui livrent en Afrique de l\'Ouest', en: 'Join thousands of merchants moving goods across West Africa' },
  'register.driverSubtitle': { fr: 'Rejoignez le réseau de livreurs ArgiDrop', en: 'Join the ArgiDrop driver network' },
  'register.businessName': { fr: 'Nom de l\'entreprise *', en: 'Business name *' },
  'register.firstName': { fr: 'Prénom *', en: 'First name *' },
  'register.lastName': { fr: 'Nom', en: 'Last name' },
  'register.email': { fr: 'Adresse e-mail *', en: 'Email address *' },
  'register.phone': { fr: 'Numéro de téléphone', en: 'Phone number' },
  'register.password': { fr: 'Mot de passe *', en: 'Password *' },
  'register.passwordPlaceholder': { fr: 'Au moins 8 caractères', en: 'At least 8 characters' },
  'register.hideReferral': { fr: 'Masquer le code de parrainage', en: 'Hide referral code' },
  'register.haveReferral': { fr: 'Vous avez un code de parrainage ?', en: 'Have a referral code?' },
  'register.referral': { fr: 'Code de parrainage', en: 'Referral code' },
  'register.submitMerchant': { fr: 'Créer le compte marchand', en: 'Create merchant account' },
  'register.submitDriver': { fr: 'Créer le compte livreur', en: 'Create driver account' },
  'register.alreadyAccount': { fr: 'Vous avez déjà un compte ?', en: 'Already have an account?' },
  'register.signin': { fr: 'Se connecter', en: 'Sign in' },
  'register.requiredFields': { fr: 'Champs requis', en: 'Required fields' },
  'register.requiredMsg': { fr: 'Le prénom, l\'e-mail et le mot de passe sont requis', en: 'First name, email, and password are required' },
  'register.businessRequired': { fr: 'Le nom de l\'entreprise est requis', en: 'Business name is required' },
  'register.passwordTooShort': { fr: 'Mot de passe trop court', en: 'Password too short' },
  'register.passwordTooShortMsg': { fr: 'Le mot de passe doit comporter au moins 8 caractères', en: 'Password must be at least 8 characters' },
  'register.failed': { fr: 'Échec de l\'inscription', en: 'Registration failed' },
  'register.tryAgain': { fr: 'Veuillez réessayer', en: 'Please try again' },

  // ─── DRIVER HOME ───
  'driverHome.hello': { fr: 'Bonjour, {name}', en: 'Hello, {name}' },
  'driverHome.online': { fr: 'En ligne', en: 'Online' },
  'driverHome.offline': { fr: 'Hors ligne', en: 'Offline' },
  'driverHome.activeLabel': { fr: 'LIVRAISON EN COURS', en: 'ACTIVE DELIVERY' },
  'driverHome.continue': { fr: 'Continuer →', en: 'Continue →' },
  'driverHome.readyCashout': { fr: 'PRÊT À ENCAISSER', en: 'READY TO CASH OUT' },
  'driverHome.xofPending': { fr: 'XOF en attente', en: 'XOF pending' },
  'driverHome.endShift': { fr: 'Fin de service', en: 'End shift' },
  'driverHome.deliveriesLabel': { fr: 'LIVRAISONS', en: 'DELIVERIES' },
  'driverHome.allTime': { fr: 'depuis le début', en: 'all time' },
  'driverHome.availableNear': { fr: 'Disponibles près de vous', en: 'Available near you' },
  'driverHome.jobOne': { fr: 'job', en: 'job' },
  'driverHome.jobMany': { fr: 'jobs', en: 'jobs' },
  'driverHome.goOnline': { fr: 'Passez en ligne pour recevoir des jobs', en: 'Go online to receive jobs' },
  'driverHome.goOnlineDesc': { fr: 'Activez l\'interrupteur ci-dessus pour voir les livraisons disponibles.', en: 'Turn on the switch above to see available deliveries.' },
  'driverHome.noJobs': { fr: 'Aucun job à proximité pour le moment', en: 'No jobs nearby right now' },
  'driverHome.noJobsDesc': { fr: 'Nous vous notifierons dès qu\'un job apparaît.', en: 'We\'ll notify you the moment one appears.' },
  'driverHome.setupPayout': { fr: 'Configurez d\'abord le paiement', en: 'Set up payout first' },
  'driverHome.setupPin': { fr: 'Configurer le PIN', en: 'Set up PIN' },
  'driverHome.cantOnline': { fr: 'Impossible de passer en ligne', en: 'Can\'t go online' },
  'driverHome.couldNotStart': { fr: 'Impossible de démarrer le service', en: 'Could not start shift' },

  // ─── MERCHANT HOME ───
  'merchantHome.hello': { fr: 'Bonjour, {name}', en: 'Hello, {name}' },
  'merchantHome.thereName': { fr: 'cher partenaire', en: 'there' },
  'merchantHome.yourBusiness': { fr: 'Votre entreprise', en: 'Your business' },
  'merchantHome.send': { fr: 'Envoyer une livraison', en: 'Send a delivery' },
  'merchantHome.sendSub': { fr: 'Trouvez un livreur en quelques minutes', en: 'Get a driver in minutes' },
  'merchantHome.activeDeliveries': { fr: 'Livraisons en cours', en: 'Active deliveries' },
  'merchantHome.noActive': { fr: 'Aucune livraison en cours', en: 'No active deliveries' },
  'merchantHome.noActiveSub': { fr: 'Touchez « Envoyer une livraison » pour commencer.', en: 'Tap "Send a delivery" above to get started.' },
  'merchantHome.pickup': { fr: 'Ramassage', en: 'Pickup' },
  'merchantHome.dropoff': { fr: 'Livraison', en: 'Drop-off' },

  // ─── JOB STATUS ───
  'status.AWAITING_PAYMENT': { fr: 'En attente de paiement', en: 'Awaiting payment' },
  'status.POSTED': { fr: 'Recherche d\'un livreur', en: 'Looking for driver' },
  'status.MATCHED': { fr: 'Livreur assigné', en: 'Driver assigned' },
  'status.IN_TRANSIT': { fr: 'En route', en: 'On the way' },
  'status.DELIVERED': { fr: 'Livré', en: 'Delivered' },
  'status.COMPLETED': { fr: 'Terminé', en: 'Completed' },
  'status.CANCELLED': { fr: 'Annulé', en: 'Cancelled' },
  'status.DISPUTED': { fr: 'En litige', en: 'Disputed' },

  // ─── NEW DELIVERY ───
  'newDelivery.title': { fr: 'Nouvelle livraison', en: 'New delivery' },
  'newDelivery.pickup': { fr: 'Ramassage', en: 'Pickup' },
  'newDelivery.dropoff': { fr: 'Livraison', en: 'Drop-off' },
  'newDelivery.package': { fr: 'Colis', en: 'Package' },
  'newDelivery.urgency': { fr: 'Urgence', en: 'Urgency' },
  'newDelivery.choosePickup': { fr: 'Choisir le point de ramassage sur la carte', en: 'Choose pickup on map' },
  'newDelivery.chooseDropoff': { fr: 'Choisir le point de livraison sur la carte', en: 'Choose drop-off on map' },
  'newDelivery.contactName': { fr: 'Nom du contact (optionnel)', en: 'Contact name (optional)' },
  'newDelivery.contactNamePh': { fr: 'ex. Yawa', en: 'e.g. Yawa' },
  'newDelivery.contactPhone': { fr: 'Téléphone du contact (optionnel)', en: 'Contact phone (optional)' },
  'newDelivery.pickupNotes': { fr: 'Notes pour le ramassage (optionnel)', en: 'Pickup notes (optional)' },
  'newDelivery.pickupNotesPh': { fr: 'Code portail, étage, instructions', en: 'Gate code, floor, instructions' },
  'newDelivery.recipientName': { fr: 'Nom du destinataire *', en: 'Recipient name *' },
  'newDelivery.recipientNamePh': { fr: 'Qui reçoit le colis ?', en: 'Who receives the package?' },
  'newDelivery.recipientPhone': { fr: 'Téléphone du destinataire *', en: 'Recipient phone *' },
  'newDelivery.dropNotes': { fr: 'Notes pour la livraison (optionnel)', en: 'Drop-off notes (optional)' },
  'newDelivery.dropNotesPh': { fr: 'Numéro d\'appartement, point de repère…', en: 'Apartment number, landmark…' },
  'newDelivery.type': { fr: 'Type', en: 'Type' },
  'newDelivery.weight': { fr: 'Poids (kg, optionnel)', en: 'Weight (kg, optional)' },
  'newDelivery.weightPh': { fr: 'ex. 2,5', en: 'e.g. 2.5' },
  'newDelivery.fragile': { fr: 'Colis fragile', en: 'Fragile package' },
  'newDelivery.description': { fr: 'Description (optionnel)', en: 'Description (optional)' },
  'newDelivery.descriptionPh': { fr: 'Que contient le colis ?', en: 'What\'s inside?' },
  'newDelivery.estimated': { fr: 'Prix estimé', en: 'Estimated price' },
  'newDelivery.priceLabel': { fr: 'Prix', en: 'Price' },
  'newDelivery.quoteOnce': { fr: 'Obtenez un devis une fois les deux adresses définies', en: 'Get a quote once both locations are set' },
  'newDelivery.couldNotQuote': { fr: 'Impossible d\'obtenir un devis', en: 'Could not get a price quote' },
  'newDelivery.getQuote': { fr: 'Obtenir un devis', en: 'Get price quote' },
  'newDelivery.continuePay': { fr: 'Continuer vers le paiement', en: 'Continue to payment' },
  'newDelivery.promoCode': { fr: 'Code promo (optionnel)', en: 'Promo code (optional)' },
  'newDelivery.promoFirst': { fr: 'Obtenez un devis d\'abord', en: 'Get a price quote first' },
  'newDelivery.invalidPromo': { fr: 'Code promo invalide', en: 'Invalid promo code' },
  'newDelivery.youPay': { fr: 'Vous payez', en: 'You pay' },

  // Scheduled deliveries (Phase 1)
  'newDelivery.schedule': { fr: 'Quand ?', en: 'When?' },
  'newDelivery.scheduleNow': { fr: 'Maintenant', en: 'Now' },
  'newDelivery.scheduleLater': { fr: 'Plus tard', en: 'Schedule' },
  'newDelivery.scheduleNowSub': { fr: 'Un chauffeur sera affecté immédiatement', en: 'A driver will be matched right away' },
  'newDelivery.scheduleLaterSub': { fr: 'Choisissez date et heure (jusqu\'à 90 jours)', en: 'Pick a date and time (up to 90 days)' },
  'newDelivery.pickDay': { fr: 'Jour de ramassage', en: 'Pickup day' },
  'newDelivery.pickTime': { fr: 'Heure de ramassage', en: 'Pickup time' },
  'newDelivery.scheduledFor': { fr: 'Programmé pour', en: 'Scheduled for' },
  'newDelivery.scheduleHint': { fr: 'Annulation gratuite jusqu\'à 24 h avant. Frais de 50 % entre 24 h et 2 h, 100 % moins de 2 h avant.', en: 'Free cancellation up to 24h before. 50% fee 24h–2h, 100% under 2h before.' },
  'newDelivery.repeat': { fr: 'Répéter', en: 'Repeat' },
  'newDelivery.repeat.never': { fr: 'Une seule fois', en: 'One time' },
  'newDelivery.repeat.daily': { fr: 'Chaque jour', en: 'Every day' },
  'newDelivery.repeat.weekly': { fr: 'Chaque semaine', en: 'Every week' },
  'newDelivery.repeatHint': { fr: 'La prochaine livraison est créée et payée automatiquement depuis votre portefeuille. Arrêtez la série à tout moment.', en: 'The next delivery is created and paid automatically from your wallet. Stop the series anytime.' },
  'newDelivery.day.today': { fr: 'Aujourd\'hui', en: 'Today' },
  'newDelivery.day.tomorrow': { fr: 'Demain', en: 'Tomorrow' },

  // Driver scheduled tab
  'driverHome.tabAvailable': { fr: 'Disponibles', en: 'Available' },
  'driverHome.tabScheduled': { fr: 'Programmées', en: 'Scheduled' },
  'driverHome.scheduledTitle': { fr: 'Livraisons programmées', en: 'Scheduled deliveries' },
  'driverHome.scheduledEmpty': { fr: 'Aucune livraison programmée pour le moment', en: 'No scheduled deliveries right now' },
  'driverHome.scheduledEmptyDesc': { fr: 'Réservez à l\'avance pour garantir le job à votre horaire.', en: 'Reserve ahead to lock a job for your schedule.' },
  'driverHome.preclaim': { fr: 'Réserver', en: 'Reserve' },
  'driverHome.preclaimed': { fr: 'Réservé', en: 'Reserved' },
  'driverHome.releasePreclaim': { fr: 'Libérer', en: 'Release' },
  'driverHome.preclaimSuccess': { fr: 'Réservé — vous serez affecté automatiquement à l\'ouverture du créneau.', en: 'Reserved — you\'ll be auto-assigned when the window opens.' },
  'driverHome.scheduledFor': { fr: 'Programmé pour', en: 'Scheduled for' },
  'newDelivery.promo': { fr: 'Promo', en: 'Promo' },
  // Package types
  'pkg.DOCS': { fr: 'Documents', en: 'Documents' },
  'pkg.FOOD': { fr: 'Nourriture', en: 'Food' },
  'pkg.SMALL_BOX': { fr: 'Petit colis', en: 'Small box' },
  'pkg.LARGE_BOX': { fr: 'Grand colis', en: 'Large box' },
  'pkg.OTHER': { fr: 'Autre', en: 'Other' },
  // Urgencies
  'urgency.STANDARD': { fr: 'Standard', en: 'Standard' },
  'urgency.STANDARD.sub': { fr: '~30–60 min', en: '~30–60 min' },
  'urgency.EXPRESS': { fr: 'Express', en: 'Express' },
  'urgency.EXPRESS.sub': { fr: 'Envoi prioritaire', en: 'Priority dispatch' },

  // ─── PAYMENT (delivery service) ───
  'payment.title': { fr: 'Paiement de la livraison', en: 'Delivery payment' },
  'payment.amountDue': { fr: 'Montant à payer', en: 'Amount due' },
  'payment.subtotal': { fr: 'Sous-total', en: 'Subtotal' },
  'payment.promo': { fr: 'Promo', en: 'Promo' },
  'payment.method': { fr: 'Mode de paiement', en: 'Choose payment method' },
  'payment.wallet': { fr: 'Portefeuille ArgiDrop', en: 'ArgiDrop wallet' },
  'payment.walletBalance': { fr: 'Solde disponible', en: 'Available balance' },
  'payment.walletRecurringNote': { fr: 'Les livraisons récurrentes sont payées depuis votre portefeuille.', en: 'Recurring deliveries are paid from your wallet.' },
  'payment.noProviders': { fr: 'Aucun mode de paiement disponible pour votre pays.', en: 'No providers available for your country.' },
  'payment.providersError': { fr: 'Impossible de charger les modes de paiement', en: 'Could not load payment providers' },
  'payment.phoneLabel': { fr: 'Numéro de téléphone à débiter', en: 'Phone number to pay from' },
  'payment.pickProvider.title': { fr: 'Choisissez un mode', en: 'Pick a provider' },
  'payment.pickProvider.body': { fr: 'Sélectionnez un mode de paiement', en: 'Choose a payment method' },
  'payment.phoneRequired.title': { fr: 'Téléphone requis', en: 'Phone required' },
  'payment.phoneRequired.body': { fr: 'Saisissez le numéro à débiter', en: 'Enter the phone number to pay from' },
  'payment.cta': { fr: 'Payer {amount} {currency}', en: 'Pay {amount} {currency}' },
  'payment.notInitiated': { fr: 'Le paiement n\'a pas démarré. Veuillez réessayer.', en: 'Payment was not initiated. Please try again.' },
  'payment.createFailed': { fr: 'Impossible de créer la livraison', en: 'Could not create the delivery' },
  'payment.failedTitle': { fr: 'Paiement échoué', en: 'Payment failed' },
  'payment.failedBody': { fr: 'Le paiement n\'a pas abouti. Vous pouvez réessayer.', en: 'The payment was not completed. You can try again.' },
  'payment.cancelTitle': { fr: 'Annuler le paiement ?', en: 'Cancel payment?' },
  'payment.cancelBody': { fr: 'La livraison ne sera pas publiée aux livreurs tant que le paiement n\'est pas confirmé.', en: 'The delivery will not be posted to drivers until payment is confirmed.' },
  'payment.keepWaiting': { fr: 'Continuer à attendre', en: 'Keep waiting' },
  'payment.legal': { fr: 'Vous serez redirigé vers la page sécurisée de l\'opérateur. Votre livraison est publiée aux livreurs dès la confirmation du paiement.', en: 'You\'ll be redirected to the provider\'s secure checkout. Your delivery will go live to drivers as soon as the payment is confirmed.' },
  'payment.complete': { fr: 'Finaliser le paiement', en: 'Complete payment' },
  'payment.waiting': { fr: 'En attente de confirmation du paiement…', en: 'Waiting for payment confirmation…' },
  'payment.loadingCheckout': { fr: 'Chargement de la page de paiement…', en: 'Loading checkout…' },

  // ─── LANGUAGE SWITCHER ───
  'lang.switch': { fr: 'Langue', en: 'Language' },
  'lang.fr': { fr: 'Français', en: 'French' },
  'lang.en': { fr: 'Anglais', en: 'English' },
  'lang.switchTo': { fr: 'Passer en {lang}', en: 'Switch to {lang}' },

  // ─── MERCHANT MORE / SETTINGS ───
  'more.section.business': { fr: 'Entreprise', en: 'Business' },
  'more.section.grow': { fr: 'Développement', en: 'Grow' },
  'more.section.account': { fr: 'Compte', en: 'Account' },
  'more.businessProfile': { fr: 'Profil de l\'entreprise', en: 'Business profile' },
  'more.businessProfileHint': { fr: 'Nom, adresse, identifiant fiscal', en: 'Name, address, tax ID' },
  'more.team': { fr: 'Membres de l\'équipe', en: 'Team members' },
  'more.teamHint': { fr: 'Inviter des collègues à publier des livraisons', en: 'Invite people who can post deliveries' },
  'more.catalog': { fr: 'Catalogue', en: 'Catalog' },
  'more.catalogHint': { fr: 'Gérer vos produits et services', en: 'Manage products and services' },
  'more.invoices': { fr: 'Factures', en: 'Invoices' },
  'more.invoicesHint': { fr: 'Télécharger les factures mensuelles', en: 'Download monthly invoices' },
  'more.invite': { fr: 'Parrainez & gagnez', en: 'Invite & earn' },
  'more.inviteHint': { fr: 'Une livraison gratuite pour chaque boutique parrainée', en: 'A free delivery for each shop you bring' },
  'more.personal': { fr: 'Informations personnelles', en: 'Personal info' },
  'more.password': { fr: 'Changer le mot de passe', en: 'Change password' },
  'more.language': { fr: 'Langue', en: 'Language' },
  'more.support': { fr: 'Aide & support', en: 'Help & support' },
  'more.signOut': { fr: 'Se déconnecter', en: 'Sign out' },
  'more.signOut.confirmTitle': { fr: 'Se déconnecter', en: 'Sign out' },
  'more.signOut.confirmBody': { fr: 'Voulez-vous vraiment vous déconnecter ?', en: 'Are you sure you want to sign out?' },

  // ─── BUSINESS SETUP / ONBOARDING ───
  'biz.setup.brandSub': { fr: 'Configuration entreprise', en: 'Business setup' },
  'biz.setup.title': { fr: 'Parlez-nous de votre entreprise', en: 'Tell us about your business' },
  'biz.setup.subtitle': { fr: 'Ces informations apparaissent sur vos demandes de livraison pour que les livreurs et destinataires sachent qui vous êtes.', en: 'This goes on your delivery requests so drivers and recipients know who you are.' },
  'biz.field.companyName': { fr: 'Nom de l\'entreprise *', en: 'Business name *' },
  'biz.field.companyNamePh': { fr: 'Boutique Akossiwa', en: 'Boutique Akossiwa' },
  'biz.field.businessType': { fr: 'Type d\'activité', en: 'Business type' },
  'biz.field.businessTypePh': { fr: 'Commerce, restaurant, pharmacie…', en: 'Retail, restaurant, pharmacy…' },
  'biz.field.taxId': { fr: 'Identifiant fiscal (optionnel)', en: 'Tax ID (optional)' },
  'biz.field.taxIdPh': { fr: 'TG12345678', en: 'TG12345678' },
  'biz.field.country': { fr: 'Pays *', en: 'Country *' },
  'biz.field.countryPh': { fr: 'Sélectionner un pays', en: 'Select country' },
  'biz.field.city': { fr: 'Ville *', en: 'City *' },
  'biz.field.cityPh': { fr: 'ex. Lomé, Cotonou, Abidjan', en: 'e.g. Lomé, Cotonou, Abidjan' },
  'biz.field.address': { fr: 'Adresse *', en: 'Address *' },
  'biz.field.addressPh': { fr: 'Rue, quartier, points de repère', en: 'Street, neighborhood, landmarks' },
  'biz.missingTitle': { fr: 'Informations manquantes', en: 'Missing information' },
  'biz.missingBody': { fr: 'Veuillez remplir : {fields}.', en: 'Please fill in: {fields}.' },
  'biz.saveFailedTitle': { fr: 'Échec de l\'enregistrement', en: 'Save failed' },
  'biz.saveFailedBody': { fr: 'Veuillez réessayer.', en: 'Please try again.' },

  // ─── BUSINESS PROFILE (settings) ───
  'bizProfile.title': { fr: 'Profil de l\'entreprise', en: 'Business profile' },
  'bizProfile.subtitle': { fr: 'Mettez à jour les informations affichées sur vos livraisons.', en: 'Update the information shown on your deliveries.' },
  'bizProfile.saved': { fr: 'Profil mis à jour', en: 'Profile updated' },

  // ─── PERSONAL INFO ───
  'personal.title': { fr: 'Informations personnelles', en: 'Personal info' },
  'personal.subtitle': { fr: 'Ces informations sont visibles par notre équipe support.', en: 'This information is visible to our support team.' },
  'personal.firstName': { fr: 'Prénom', en: 'First name' },
  'personal.lastName': { fr: 'Nom', en: 'Last name' },
  'personal.email': { fr: 'Adresse e-mail', en: 'Email address' },
  'personal.emailLocked': { fr: 'Pour modifier votre e-mail, contactez le support.', en: 'To change your email, contact support.' },
  'personal.phone': { fr: 'Téléphone', en: 'Phone number' },
  'personal.save': { fr: 'Enregistrer les modifications', en: 'Save changes' },
  'personal.saved': { fr: 'Informations enregistrées', en: 'Saved' },
  'personal.savedBody': { fr: 'Vos informations ont été mises à jour.', en: 'Your details have been updated.' },
  'personal.errorTitle': { fr: 'Erreur', en: 'Error' },
  'personal.errorBody': { fr: 'Impossible d\'enregistrer. Veuillez réessayer.', en: 'Could not save. Please try again.' },
  'personal.phoneInUse': { fr: 'Ce numéro est déjà utilisé.', en: 'This phone number is already in use.' },

  // ─── CHANGE PASSWORD ───
  'pwd.title': { fr: 'Changer le mot de passe', en: 'Change password' },
  'pwd.subtitle': { fr: 'Choisissez un mot de passe d\'au moins 8 caractères.', en: 'Pick a new password with at least 8 characters.' },
  'pwd.current': { fr: 'Mot de passe actuel', en: 'Current password' },
  'pwd.new': { fr: 'Nouveau mot de passe', en: 'New password' },
  'pwd.confirm': { fr: 'Confirmer le nouveau mot de passe', en: 'Confirm new password' },
  'pwd.update': { fr: 'Mettre à jour', en: 'Update password' },
  'pwd.successTitle': { fr: 'Mot de passe mis à jour', en: 'Password updated' },
  'pwd.successBody': { fr: 'Votre mot de passe a bien été changé.', en: 'Your password has been changed.' },
  'pwd.requiredTitle': { fr: 'Champs requis', en: 'Required fields' },
  'pwd.requiredBody': { fr: 'Veuillez remplir tous les champs.', en: 'Please fill in all fields.' },
  'pwd.tooShort': { fr: 'Le nouveau mot de passe doit comporter au moins 8 caractères.', en: 'New password must be at least 8 characters.' },
  'pwd.mismatchTitle': { fr: 'Mots de passe différents', en: 'Passwords don\'t match' },
  'pwd.mismatchBody': { fr: 'La confirmation ne correspond pas au nouveau mot de passe.', en: 'Confirmation does not match the new password.' },
  'pwd.wrongCurrent': { fr: 'Mot de passe actuel incorrect.', en: 'Current password is incorrect.' },
  'pwd.errorTitle': { fr: 'Erreur', en: 'Error' },

  // ─── LANGUAGE PICKER SCREEN ───
  'langScreen.title': { fr: 'Choisir la langue', en: 'Choose language' },
  'langScreen.subtitle': { fr: 'Toute l\'application sera affichée dans cette langue.', en: 'The whole app will be shown in this language.' },
  'langScreen.saved': { fr: 'Langue enregistrée', en: 'Language saved' },
  'langScreen.fr.label': { fr: 'Français', en: 'French' },
  'langScreen.en.label': { fr: 'Anglais', en: 'English' },
  'langScreen.fr.note': { fr: 'Recommandé pour le Togo et la zone CEDEAO', en: 'Recommended for Togo and the CEDEAO region' },
  'langScreen.en.note': { fr: 'Recommandé pour le Ghana, le Nigeria et la Sierra Leone', en: 'Recommended for Ghana, Nigeria, and Sierra Leone' },

  // ─── SUPPORT ───
  'support.title': { fr: 'Aide & support', en: 'Help & support' },
  'support.subtitle': { fr: 'Notre équipe est basée à Lomé et répond du lundi au samedi, de 8h à 19h (GMT).', en: 'Our team is based in Lomé and replies Monday to Saturday, 8am – 7pm (GMT).' },
  'support.email': { fr: 'Nous écrire par e-mail', en: 'Email support' },
  'support.emailHint': { fr: 'support@argidrop.com — réponse sous 24 h ouvrées', en: 'support@argidrop.com — reply within 24 business hours' },
  'support.faq': { fr: 'Questions fréquentes', en: 'Frequently asked questions' },
  'support.faqHint': { fr: 'Consultez les réponses aux questions courantes', en: 'See answers to common questions' },
  'support.privacy': { fr: 'Politique de confidentialité', en: 'Privacy policy' },
  'support.terms': { fr: 'Conditions d\'utilisation', en: 'Terms of service' },
  'support.deletion': { fr: 'Supprimer mon compte', en: 'Delete my account' },
  'support.legal': { fr: 'ArgiDrop est exploité par Argilette LLC (St. Louis, MO, USA) avec une équipe d\'opérations à Lomé, Togo.', en: 'ArgiDrop is operated by Argilette LLC (St. Louis, MO, USA) with an operations team in Lomé, Togo.' },

  'more.deleteAccount': { fr: 'Supprimer mon compte', en: 'Delete account' },
  'more.deleteAccountHint': { fr: 'Supprimer définitivement votre compte et vos données', en: 'Permanently delete your account and data' },

  'delete.title': { fr: 'Supprimer mon compte', en: 'Delete my account' },
  'delete.warning': { fr: 'Cette action est définitive. Une fois supprimé, votre compte ne peut pas être restauré.', en: 'This action is permanent. Once deleted, your account cannot be restored.' },
  'delete.willRemove': { fr: 'Ce qui sera supprimé', en: 'What will be removed' },
  'delete.bullet.profile': { fr: 'Votre profil, votre nom, votre e-mail et votre téléphone', en: 'Your profile, name, email and phone number' },
  'delete.bullet.history': { fr: 'L\'historique des livraisons rattaché à votre compte', en: 'The delivery history attached to your account' },
  'delete.bullet.docs': { fr: 'Vos documents KYC et photos de profil', en: 'Your KYC documents and profile photos' },
  'delete.bullet.notif': { fr: 'Vos notifications, jetons d\'appareil et sessions ouvertes', en: 'Your notifications, device tokens and active sessions' },
  'delete.confirmLabel': { fr: 'Confirmer avec votre mot de passe', en: 'Confirm with your password' },
  'delete.confirmHelper': { fr: 'Pour des raisons de sécurité, veuillez saisir votre mot de passe pour confirmer la suppression.', en: 'For security, please enter your password to confirm deletion.' },
  'delete.passwordPlaceholder': { fr: 'Mot de passe', en: 'Password' },
  'delete.passwordRequired': { fr: 'Veuillez saisir votre mot de passe.', en: 'Please enter your password.' },
  'delete.cta': { fr: 'Supprimer mon compte définitivement', en: 'Permanently delete my account' },
  'delete.cancel': { fr: 'Annuler', en: 'Cancel' },
  'delete.confirmTitle': { fr: 'Êtes-vous sûr ?', en: 'Are you sure?' },
  'delete.confirmMsg': { fr: 'Cette action est irréversible. Votre compte et vos données personnelles seront supprimés immédiatement.', en: 'This action cannot be undone. Your account and personal data will be deleted immediately.' },
  'delete.confirmCta': { fr: 'Oui, supprimer', en: 'Yes, delete' },
  'delete.doneTitle': { fr: 'Compte supprimé', en: 'Account deleted' },
  'delete.doneMsg': { fr: 'Votre compte a été supprimé. Merci d\'avoir utilisé ArgiDrop.', en: 'Your account has been deleted. Thank you for using ArgiDrop.' },
  'delete.error': { fr: 'Impossible de supprimer le compte. Veuillez réessayer.', en: 'Could not delete the account. Please try again.' },
  'delete.legal': { fr: 'Conformément au RGPD et aux exigences d\'Apple, certaines données peuvent être conservées pour des raisons légales (facturation, audit anti-fraude) sous forme anonymisée.', en: 'In accordance with GDPR and Apple requirements, some data may be retained in anonymized form for legal reasons (billing, anti-fraud audit).' },
  'support.cantOpen': { fr: 'Impossible d\'ouvrir le lien', en: 'Could not open link' },

  // ─── ROADMAP / COMING SOON SCREEN ───
  'roadmap.team.title': { fr: 'Membres de l\'équipe', en: 'Team members' },
  'roadmap.team.body': { fr: 'Bientôt, vous pourrez inviter vos collègues à publier des livraisons depuis votre compte entreprise, avec leurs propres identifiants et un suivi individuel des activités.', en: 'Soon you\'ll be able to invite colleagues to post deliveries from your business account, with their own logins and individual activity tracking.' },
  'roadmap.catalog.title': { fr: 'Catalogue de produits', en: 'Product catalog' },
  'roadmap.catalog.body': { fr: 'Nous préparons un catalogue qui pré-remplira automatiquement vos descriptions de colis : produits favoris, prix, photos.', en: 'We\'re preparing a catalog that will auto-fill your package descriptions: favorite products, prices, photos.' },
  'roadmap.invoices.title': { fr: 'Factures mensuelles', en: 'Monthly invoices' },
  'roadmap.invoices.body': { fr: 'Téléchargez vos factures PDF pour la comptabilité, avec TVA détaillée et historique complet par mois.', en: 'Download PDF invoices for your accounting, with itemized VAT and full per-month history.' },
  'roadmap.eta': { fr: 'Disponible courant 2026', en: 'Coming in 2026' },
  'roadmap.notify': { fr: 'Vous serez notifié·e dès que cette fonctionnalité sera disponible.', en: 'You\'ll be notified the moment this feature is available.' },
  'roadmap.contactSupport': { fr: 'Besoin de cette fonction maintenant ? Écrivez-nous.', en: 'Need this feature now? Get in touch.' },

  // ─── BOTTOM TAB BAR LABELS ───
  'tab.merchant.home': { fr: 'Accueil', en: 'Home' },
  'tab.merchant.history': { fr: 'Historique', en: 'History' },
  'tab.merchant.new': { fr: 'Envoyer', en: 'New' },
  'tab.merchant.more': { fr: 'Plus', en: 'More' },
  'tab.driver.home': { fr: 'Accueil', en: 'Home' },
  'tab.driver.earnings': { fr: 'Gains', en: 'Earnings' },
  'tab.driver.notifications': { fr: 'Notifications', en: 'Notifications' },
  'tab.driver.profile': { fr: 'Profil', en: 'Profile' },

  // ─── CONSUMER (individual customers) ───
  'role.consumer.title': { fr: 'Je commande', en: 'I order' },
  'role.consumer.desc': { fr: 'Repas, boutiques et courses en ville, livrés chez vous', en: 'Food, shops and rides around town, delivered to you' },
  'register.consumerSub': { fr: 'Compte client', en: 'Customer account' },
  'register.consumerSubtitle': { fr: 'Créez votre compte pour commander des repas, faire vos achats et réserver des courses.', en: 'Create your account to order food, shop and book rides.' },
  'register.submitConsumer': { fr: 'Créer mon compte', en: 'Create my account' },

  'tab.consumer.home': { fr: 'Accueil', en: 'Home' },
  'tab.consumer.food': { fr: 'Repas', en: 'Food' },
  'tab.consumer.shop': { fr: 'Achats', en: 'Shop' },
  'tab.consumer.rides': { fr: 'Courses', en: 'Rides' },
  'tab.consumer.more': { fr: 'Plus', en: 'More' },

  'consumer.greeting': { fr: 'Bonjour', en: 'Hello' },
  'consumer.homeSubtitle': { fr: 'Que voulez-vous aujourd\'hui ?', en: 'What would you like today?' },
  'consumer.actionFood': { fr: 'Commander un repas', en: 'Order food' },
  'consumer.actionFoodDesc': { fr: 'Restaurants près de vous', en: 'Restaurants near you' },
  'consumer.actionShops': { fr: 'Faire des achats', en: 'Shop' },
  'consumer.actionShopsDesc': { fr: 'Boutiques et commerçants', en: 'Shops and merchants' },
  'consumer.actionRide': { fr: 'Réserver une course', en: 'Book a ride' },
  'consumer.actionRideDesc': { fr: 'Moto, zémidjan ou voiture', en: 'Moto, zemidjan or car' },
  'consumer.restaurants': { fr: 'Restaurants', en: 'Restaurants' },
  'consumer.shops': { fr: 'Boutiques', en: 'Shops' },
  'consumer.seeAll': { fr: 'Voir tout', en: 'See all' },
  'consumer.noRestaurants': { fr: 'Aucun restaurant disponible pour le moment. Revenez bientôt !', en: 'No restaurants available yet. Check back soon!' },
  'consumer.noShops': { fr: 'Aucune boutique disponible pour le moment.', en: 'No shops available yet.' },
  'consumer.closed': { fr: 'Fermé', en: 'Closed' },
  'consumer.loadFailed': { fr: 'Impossible de charger. Tirez pour réessayer.', en: 'Could not load. Pull to retry.' },
  'consumer.min': { fr: 'min', en: 'min' },

  'consumer.foodTitle': { fr: 'Commander un repas', en: 'Order food' },
  'consumer.foodSubtitle': { fr: 'Nous livrons à votre porte', en: 'We deliver to your door' },
  'consumer.menu': { fr: 'Menu', en: 'Menu' },
  'consumer.minOrder': { fr: 'Commande min.', en: 'Min. order' },
  'consumer.deliveryFee': { fr: 'Livraison', en: 'Delivery' },
  'consumer.serviceFee': { fr: 'Frais de service', en: 'Service fee' },
  'consumer.subtotal': { fr: 'Sous-total', en: 'Subtotal' },
  'consumer.total': { fr: 'Total', en: 'Total' },
  'consumer.yourOrder': { fr: 'Votre commande', en: 'Your order' },
  'consumer.deliveryAddress': { fr: 'Adresse de livraison', en: 'Delivery address' },
  'consumer.addressPlaceholder': { fr: 'Ex. Quartier Bè, rue des Palmiers, portail vert', en: 'E.g. Bè district, Palm street, green gate' },
  'consumer.notes': { fr: 'Instructions (optionnel)', en: 'Notes (optional)' },
  'consumer.placeOrder': { fr: 'Commander', en: 'Place order' },
  'consumer.placing': { fr: 'Envoi…', en: 'Placing…' },
  'consumer.cashOnDelivery': { fr: 'Paiement à la livraison', en: 'Cash on delivery' },
  'consumer.orderPlaced': { fr: 'Commande envoyée !', en: 'Order placed!' },
  'consumer.orderPlacedBody': { fr: 'prépare votre commande. Payez en espèces à la livraison.', en: 'is preparing your order. Pay cash on delivery.' },
  'consumer.orderNumber': { fr: 'N° de commande', en: 'Order number' },
  'consumer.orderFailed': { fr: 'La commande a échoué', en: 'Order failed' },
  'consumer.addressRequired': { fr: 'Veuillez saisir votre adresse de livraison', en: 'Please enter your delivery address' },
  'consumer.belowMinOrder': { fr: 'Le minimum de commande est de', en: 'Minimum order is' },
  'consumer.emptyMenu': { fr: 'Menu indisponible pour le moment', en: 'Menu unavailable right now' },
  'consumer.payMethod': { fr: 'Paiement', en: 'Payment' },
  'consumer.payCashOption': { fr: 'Espèces à la livraison', en: 'Cash on delivery' },
  'consumer.payMomoOption': { fr: 'Mobile money', en: 'Mobile money' },
  'consumer.momoNumber': { fr: 'Numéro mobile money', en: 'Mobile money number' },
  'consumer.momoNumberRequired': { fr: 'Saisissez votre numéro mobile money', en: 'Enter your mobile money number' },
  'consumer.pickProvider': { fr: 'Choisissez un opérateur', en: 'Choose a provider' },
  'consumer.payAndOrder': { fr: 'Payer et commander', en: 'Pay & order' },
  'consumer.waitingPayment': { fr: 'En attente de la confirmation du paiement…', en: 'Waiting for payment confirmation…' },
  'consumer.paymentConfirmed': { fr: 'Paiement confirmé', en: 'Payment confirmed' },
  'consumer.backToRestaurants': { fr: 'Retour aux restaurants', en: 'Back to restaurants' },

  'invoices.title': { fr: 'Factures', en: 'Invoices' },
  'invoices.subtitle': { fr: 'Une facture par livraison payée. Touchez pour télécharger le PDF.', en: 'One invoice per paid delivery. Tap to download the PDF.' },
  'invoices.empty': { fr: 'Aucune facture pour le moment. Elles apparaissent après votre première livraison payée.', en: 'No invoices yet. They appear after your first paid delivery.' },
  'invoices.paid': { fr: 'Payée', en: 'Paid' },
  'invoices.held': { fr: 'Fonds bloqués', en: 'Funds held' },
  'invoices.saved': { fr: 'Facture téléchargée', en: 'Invoice downloaded' },
  'invoices.downloadFailed': { fr: 'Téléchargement impossible', en: 'Download failed' },

  'catalog.title': { fr: 'Catalogue', en: 'Catalog' },
  'catalog.products': { fr: 'Produits', en: 'Products' },
  'catalog.orders': { fr: 'Commandes', en: 'Orders' },
  'catalog.addProduct': { fr: 'Ajouter un produit', en: 'Add product' },
  'catalog.editProduct': { fr: 'Modifier le produit', en: 'Edit product' },
  'catalog.name': { fr: 'Nom', en: 'Name' },
  'catalog.price': { fr: 'Prix', en: 'Price' },
  'catalog.category': { fr: 'Catégorie', en: 'Category' },
  'catalog.categoryPh': { fr: 'Ex. Épicerie, Boissons…', en: 'E.g. Grocery, Drinks…' },
  'catalog.unit': { fr: 'Unité (optionnel)', en: 'Unit (optional)' },
  'catalog.description': { fr: 'Description (optionnel)', en: 'Description (optional)' },
  'catalog.inStock': { fr: 'En stock', en: 'In stock' },
  'catalog.outOfStock': { fr: 'Épuisé', en: 'Out of stock' },
  'catalog.noPrice': { fr: 'Prix non défini', en: 'No price set' },
  'catalog.save': { fr: 'Enregistrer', en: 'Save' },
  'catalog.saveFailed': { fr: 'Enregistrement impossible', en: 'Could not save' },
  'catalog.nameRequired': { fr: 'Le nom du produit est requis', en: 'Product name is required' },
  'catalog.delete': { fr: 'Supprimer', en: 'Delete' },
  'catalog.deleteTitle': { fr: 'Supprimer ce produit ?', en: 'Delete this product?' },
  'catalog.empty': { fr: 'Aucun produit. Ajoutez votre premier article pour apparaître dans la boutique.', en: 'No products yet. Add your first item to appear in the shop.' },
  'catalog.ordersEmpty': { fr: 'Aucune commande reçue pour le moment.', en: 'No orders received yet.' },
  'catalog.photoLimit': { fr: 'Limite de photos atteinte pour votre abonnement. Passez à un forfait supérieur.', en: 'Photo limit reached for your plan. Upgrade to add more.' },
  'catalog.confirm': { fr: 'Confirmer', en: 'Confirm' },
  'catalog.cancelOrder': { fr: 'Refuser', en: 'Decline' },
  'catalog.markReady': { fr: 'Prête', en: 'Ready' },
  'catalog.markPickedUp': { fr: 'Récupérée', en: 'Picked up' },
  'catalog.markDelivered': { fr: 'Livrée', en: 'Delivered' },
  'orderStatus.PENDING': { fr: 'En attente', en: 'Pending' },
  'orderStatus.CONFIRMED': { fr: 'Confirmée', en: 'Confirmed' },
  'orderStatus.PREPARING': { fr: 'En préparation', en: 'Preparing' },
  'orderStatus.READY_FOR_PICKUP': { fr: 'Prête', en: 'Ready' },
  'orderStatus.PICKED_UP': { fr: 'Récupérée', en: 'Picked up' },
  'orderStatus.DELIVERED': { fr: 'Livrée', en: 'Delivered' },
  'orderStatus.CANCELLED': { fr: 'Annulée', en: 'Cancelled' },
  'orderStatus.REFUNDED': { fr: 'Remboursée', en: 'Refunded' },

  'team.title': { fr: 'Équipe', en: 'Team' },
  'team.subtitle': { fr: 'Ajoutez des membres de votre équipe pour créer et suivre des livraisons avec leur propre compte.', en: 'Add team members who can create and track deliveries with their own account.' },
  'team.owner': { fr: 'PROPRIÉTAIRE', en: 'OWNER' },
  'team.staff': { fr: 'ÉQUIPE', en: 'STAFF' },
  'team.empty': { fr: 'Aucun membre pour le moment. Ajoutez votre premier collaborateur.', en: 'No members yet. Add your first team member.' },
  'team.add': { fr: 'Ajouter un membre', en: 'Add member' },
  'team.create': { fr: 'Créer le compte', en: 'Create account' },
  'team.fillRequired': { fr: 'Prénom, nom et email sont requis', en: 'First name, last name and email are required' },
  'team.addFailed': { fr: 'Impossible d\'ajouter le membre', en: 'Could not add member' },
  'team.removeTitle': { fr: 'Retirer ce membre ?', en: 'Remove this member?' },
  'team.removeBody': { fr: 'son compte sera désactivé.', en: 'their account will be deactivated.' },
  'team.remove': { fr: 'Retirer', en: 'Remove' },
  'team.credTitle': { fr: 'Compte créé — mot de passe temporaire', en: 'Account created — temporary password' },
  'team.credHint': { fr: 'Partagez ces identifiants en toute sécurité. Ce mot de passe ne sera plus affiché ; le membre devra le changer après sa première connexion.', en: 'Share these credentials securely. This password won\'t be shown again; the member should change it after first sign-in.' },

  'consumer.shopsTitle': { fr: 'Faire des achats', en: 'Shop' },
  'consumer.shopsSubtitle': { fr: 'Boutiques et commerçants livrés chez vous', en: 'Shops and merchants delivered to you' },
  'consumer.products': { fr: 'Produits', en: 'Products' },
  'consumer.emptyShop': { fr: 'Aucun produit disponible pour le moment', en: 'No products available right now' },
  'consumer.backToShops': { fr: 'Retour aux boutiques', en: 'Back to shops' },
  'consumer.outOfStock': { fr: 'Épuisé', en: 'Out of stock' },

  'consumer.rideTitle': { fr: 'Réserver une course', en: 'Book a ride' },
  'consumer.rideSubtitle': { fr: 'Un chauffeur vient vous chercher', en: 'A driver picks you up' },
  'consumer.rideFrom': { fr: 'Départ', en: 'Pickup' },
  'consumer.rideTo': { fr: 'Destination', en: 'Destination' },
  'consumer.choosePlace': { fr: 'Choisir sur la carte', en: 'Choose on the map' },
  'consumer.rideVehicle': { fr: 'Véhicule', en: 'Vehicle' },
  'consumer.ridePrice': { fr: 'Prix estimé', en: 'Estimated price' },
  'consumer.rideDistance': { fr: 'Distance', en: 'Distance' },
  'consumer.rideDrivers': { fr: 'chauffeurs disponibles', en: 'drivers available' },
  'consumer.rideNoDrivers': { fr: 'Aucun chauffeur disponible pour ce véhicule actuellement', en: 'No drivers available for this vehicle right now' },
  'consumer.estimating': { fr: 'Calcul du prix…', en: 'Estimating…' },
  'consumer.rideBook': { fr: 'Réserver', en: 'Book ride' },
  'consumer.booking': { fr: 'Réservation…', en: 'Booking…' },
  'consumer.payCash': { fr: 'Paiement en espèces au chauffeur', en: 'Pay the driver in cash' },
  'consumer.rideBookedSearching': { fr: 'Course réservée ! Recherche d\'un chauffeur…', en: 'Ride booked! Searching for a driver…' },
  'consumer.rideBookedMatched': { fr: 'Course réservée ! Un chauffeur arrive.', en: 'Ride booked! A driver is on the way.' },
  'consumer.rideFailed': { fr: 'Impossible de traiter la demande', en: 'Could not process the request' },
  'consumer.fillAddresses': { fr: 'Choisissez un départ et une destination', en: 'Choose a pickup point and a destination' },

  'consumer.more.title': { fr: 'Mon compte', en: 'My account' },
};

/**
 * Get a translated string. Supports {name} interpolation.
 * @param {string} key - string key
 * @param {string} lang - 'fr' | 'en'
 * @param {object} [vars] - optional interpolation map { name: 'Kodjo' }
 * @returns {string}
 */
export function t(key, lang = 'fr', vars) {
  const entry = strings[key];
  if (!entry) return key;
  let str = entry[lang] || entry.en || key;
  if (vars) {
    Object.keys(vars).forEach(k => {
      str = str.replace(new RegExp(`\\{${k}\\}`, 'g'), vars[k]);
    });
  }
  return str;
}

/**
 * Get the user's preferred language from their profile.
 * Falls back to 'fr' (francophone West Africa default).
 */
export function getLang(user) {
  return user?.language === 'en' ? 'en' : 'fr';
}

export default { t, getLang };
