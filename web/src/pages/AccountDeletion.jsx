...
export default function AccountDeletion() {
  ...
  const sections = t('legal.accountDeletion.sections', { returnObjects: true });
    ...
    <div style={{ minHeight:'100vh', background:C.cream, fontFamily:'Inter, system-ui, sans-serif', color:C.ink }}>
      ...
      <nav style={{ position:'sticky', top:0, zIndex:100, background:C.paper, borderBottom:`1px solid ${C.border}`, padding:'16px 40px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <Link to="/" style={{ textDecoration:'none', display:'flex', alignItems:'center', gap:10 }}>
          <img src="/argidrop-icon.png" alt="" style={{ width:28, height:28, borderRadius:6 }} />
        ...
        <div style={{ display:'flex', gap:18, alignItems:'center' }}>
          <Link to="/privacy" style={{ color:C.muted, textDecoration:'none', fontSize:13, fontWeight:500 }}>{t('landing.footer.privacy')}</Link>
          <Link to="/terms" style={{ color:C.muted, textDecoration:'none', fontSize:13, fontWeight:500 }}>{t('landing.footer.terms')}</Link>
      ...
      <article style={{ maxWidth:760, margin:'0 auto', padding:'64px 40px 96px' }}>
        ...
        <h1 style={{ fontFamily:'Fraunces, serif', fontSize:48, fontWeight:400, lineHeight:1.1, letterSpacing:'-0.02em', margin:'0 0 16px' }}>{t('legal.accountDeletion.title')}</h1>
        <div style={{ fontSize:13, color:C.muted, marginBottom:16 }}>{t('legal.lastUpdated')}</div>
        <p style={{ fontSize:16, lineHeight:1.65, color:C.ink, margin:'0 0 48px' }}>{t('legal.accountDeletion.lead')}</p>
        ...
        <div style={{ marginTop:64, padding:24, background:C.paper, border:`1px solid ${C.border}`, borderRadius:6 }}>
          <div style={{ fontSize:11, color:C.bronze, letterSpacing:'0.16em', fontWeight:500, textTransform:'uppercase', marginBottom:8 }}>{t('legal.accountDeletion.contactTitle')}</div>
          <a href="mailto:privacy@argidrop.com?subject=Account%20deletion%20request" style={{ fontFamily:'Fraunces, serif', fontSize:24, fontWeight:500, color:C.forest, textDecoration:'none' }}>privacy@argidrop.com</a>
          <div style={{ fontSize:13, color:C.muted, marginTop:12, lineHeight:1.6 }}>{t('legal.accountDeletion.contactNote')}</div>
        </div>
      </article>
    </div>
  );
