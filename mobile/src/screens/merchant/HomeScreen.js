...
export default function MerchantHomeScreen({ navigation }) {
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        <View>
          <Text style={s.greeting}>Hello, {user?.firstName || 'there'}</Text>
          <Text style={s.companyName}>{user?.businessProfile?.companyName || 'Your business'}</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 12 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={C.forest} />}>
        <TouchableOpacity style={s.cta} activeOpacity={0.9} onPress={() => navigation.navigate('NewDelivery')}>
          <View style={{ flex: 1 }}>
            <Text style={s.ctaTitle}>Send a delivery</Text>
            <Text style={s.ctaSub}>Get a driver in minutes</Text>
          </View>
        ...
        {loading ? <ActivityIndicator color={C.forest} style={{ marginTop: 24 }} /> :
         active.length === 0 ? (
          <View style={s.empty}>
            ...
            <Text style={s.emptySub}>Tap "Send a delivery" above to get started.</Text>
          </View>
         ) : active.map(job => (
          <TouchableOpacity key={job.id} style={s.jobCard} onPress={() => navigation.navigate('JobDetail', { jobId: job.id })} activeOpacity={0.85}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <Text style={[s.jobStatus, { color: STATUS_COLOR[job.status] || C.muted }]}>{STATUS_LABELS[job.status] || job.status}</Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
              <Ionicons name="ellipse" size={8} color={C.bronze} />
              <Text style={s.jobAddr} numberOfLines={1}>{job.pickupAddress || 'Pickup'}</Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Ionicons name="location" size={10} color={C.forest} />
              <Text style={s.jobAddr} numberOfLines={1}>{job.dropoffAddress || 'Drop-off'}</Text>
            </View>
          </TouchableOpacity>
         ))}
      </ScrollView>
